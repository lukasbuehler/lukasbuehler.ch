...
export async function publishedEntries() {
  ...
  for (const entry of entries) {
    for (const target of linkedPaths(entry)) {
      if (!known.has(target))
        throw new Error(
          `${entry.id} links to missing or unpublished entry ${target}`,
        );
    }
  }
  return entries.sort(
...
export function linkedPaths(entry: Entry) {
  ...
  const paths = [
    ...body.matchAll(
      /\[\[((?:notes|projects)\/[a-z0-9-]+)(?:\|[^\]]+)?\]\]|\]\(\/?((?:notes|projects)\/[a-z0-9-]+)\/?(?:#[^\s)]*)?\)/g,
    ),
  ];
  return new Set(paths.map((match) => `/${match[1] ?? match[2]}/`));
}
export const backlinks = (entry: Entry, entries: Entry[]) =>
  entries.filter(
    (other) => other.id !== entry.id && linkedPaths(other).has(entryUrl(entry)),
  );
